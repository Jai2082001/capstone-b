...
exports.saveCaseUser = (req, res, next) => {
    ...
    const NewCase = new Case({ name: name, description: description, country: country, province: province, city: city, addedBy: id, imageUrl: image, phonenumber: phonenumber, signid: 'N/A', status: 'No' })
...
exports.displayAllCaseUser = (req, res, next) => {
    console.log('DISPLAYALLCASEUSER')
    const {userid}  = req.headers;
    const responseArray = [];
    Case.find({}).then((response)=>{
        ...
        if(response.length>0){
            for(let i=0;i<response.length;i++){
                if(response.addedBy == userid){
                    continue;
                }
                responseArray.push(response[i]);
            }
            res.send({array: responseArray})
        }else{
            res.send({array: []})
        }
    })
