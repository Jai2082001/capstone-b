...
exports.saveShelter = (req, res, next) => {
  ...
  const { shelterName, shelterOrganization, shelterDescription, sNeed, sPresent, image, addedby } = req.body;
  if (addedby == 'Admin') {
    Shelter.findOne({ ShelterName: shelterName }).then((response) => {
      if (!response) {
        const ShelterInstance = new Shelter({ ShelterName: shelterName, ShelterOrganization: shelterOrganization, ShelterDescription: shelterDescription, ResidentCapacity: sNeed, ResidentPresentMember: sPresent, ImageUrl: image, AddedBy: 'Admin' })
